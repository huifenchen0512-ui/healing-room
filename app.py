...
st.markdown("""
    ...
    .stApp { background-color: #FAF9F6; font-family: 'Noto Serif TC', serif; }
    ...
    .custom-card {
        ...
        box-shadow: 0 10px 30px rgba(45, 70, 62, 0.05); border: 1px solid #E9E4D9;
        margin-bottom: 2rem; transition: all 0.3s ease;
    }
    .custom-card:hover { box-shadow: 0 15px 45px rgba(166, 123, 91, 0.1); transform: translateY(-5px); }

    .announcement-box {
        background: linear-gradient(135deg, #2D463E 0%, #1A2E28 100%);
        color: #FAF9F6; padding: 1.5rem; border-radius: 16px; margin-bottom: 2rem;
    }

    .stButton>button { 
        background: linear-gradient(135deg, #A67B5B 0%, #8B6B4D 100%);
        color: #ffffff; width: 100%; border-radius: 16px; 
        height: 4em; font-weight: 700; border: none; font-size: 1.1em; letter-spacing: 2px;
    }
    .stButton>button:hover { background: linear-gradient(135deg, #2D463E 0%, #1A2E28 100%); }
    ...
    .tarot-card {
        ...
        box-shadow: 0 20px 50px rgba(0,0,0,0.1); border: 2px solid #A67B5B;
...
<div class="announcement-box">
    ...
    • 2/7 梯次：三重 (19:00+) / 2/8 華視 (11:00-17:00)<br>
...
with col1:
    ...
    st.image("548282507_1196129685655556_143484642680713398_n.jpg", caption="癒室手工焦糖核桃，溫暖靈魂的味道。", use_container_width=True)
    st.subheader("📝 預約資訊")
    c_name = st.text_input("您的稱呼")
    c_phone = st.text_input("聯絡電話", placeholder="例：0912345678", key="final_phone")
    ...
    u_g = pd.to_numeric(batch['經典數量']).sum()
    u_w = pd.to_numeric(batch['核桃數量']).sum()
    ...
    loc_opt = st.selectbox("領取安排", pickup_opts)
...
with col2:
    ...
    q_g = st.number_input("購買數量 (經典)", min_value=0, max_value=rem_g, step=1, key="q_g_18")
    st.write(f"**純焦桃核桃** (剩餘 {rem_w} 盒)")
    st.progress(min(1.0, u_w / max_w if max_w > 0 else 1))
    q_w = st.number_input("購買數量 (核桃)", min_value=0, max_value=rem_w, step=1, key="q_w_18")
    st.markdown(f"### 💰 預算總額：NT$ {(q_g + q_w) * 190}")
...
if st.button("✨ 送出預約，並領取今日療癒指引 ✨"):
    if c_name and c_phone and (q_g + q_w) > 0:
        with st.spinner("打雜小妹正在洗牌..."):
            ...
            tarot_deck = [
                {"e": "air", "i": "🃏", "t": "0 愚者", "d": "新冒險的開始，為您的熱情鼓掌！"},
                {"e": "fire", "i": "🪄", "t": "I 魔術師", "d": "您擁有一切創造美好生活的力量。"},
                {"e": "water", "i": "📜", "t": "II 女教皇", "d": "靜聽直覺，它是您最準確的導播。"},
                {"e": "earth", "i": "👑", "t": "III 女皇", "d": "今日適合被美好與豐盛包圍。"},
                {"e": "fire", "i": "🏛️", "t": "IV 皇帝", "d": "穩定掌控，主導生活的節奏。"},
                {"e": "earth", "i": "🙏", "t": "V 教皇", "d": "智慧藏於傳統與經驗之中。"},
                {"e": "air", "i": "💞", "t": "VI 戀人", "d": "美好的連結正悄悄發生。"},
                {"e": "fire", "i": "🛒", "t": "VII 戰車", "d": "衝勁十足！今日無人能擋。"},
                {"e": "fire", "i": " Lions", "i": "🦁", "t": "VIII 力量", "d": "溫柔的堅韌，足以平定喧囂。"},
                {"e": "earth", "i": "💡", "t": "IX 隱者", "d": "獨處時光中，找回真實的自己。"},
                {"e": "fire", "i": "🎡", "t": "X 命運之輪", "d": "轉機已至，好運隨香而來。"},
                ...
                {"e": "fire", "i": "⚡", "t": "XVI 高塔", "d": "突破性的重組，為了更穩固的未來。"},
                {"e": "air", "i": "⭐", "t": "XVII 星星", "d": "希望星光指引，願望正慢慢熟成。"},
                {"e": "water", "i": "🌙", "t": "XVIII 月亮", "d": "擁抱不安，星光會指引明早的路。"},
                {"e": "fire", "i": "☀️", "t": "XIX 太陽", "d": "充滿活力的明亮光芒，今日圓滿。"},
                {"e": "fire", "i": "🎺", "t": "XX 審判", "d": "聽從內心呼喚，再次覺醒啟航。"},
                {"e": "earth", "i": "🌍", "t": "XXI 世界", "d": "圓滿的達成，最好的犒賞。"}
            ]
            drawn = random.choice(tarot_deck)
            st.balloons()
            st.success("✅ 預約成功！請領取您的療癒指引：")
            pytime.sleep(0.5)
            st.markdown(f"<div class='tarot-container'><div class='tarot-card {drawn['e']}'><div class='tarot-icon'>{drawn['i']}</div><div style='font-size:1.4rem;font-weight:700;margin-bottom:10px'>{drawn['t']}</div><p>{drawn['d']}</p><small>— 來自 癒室 - 手工甜點 的專屬祝福</small></div></div>", unsafe_allow_html=True)
    else:
